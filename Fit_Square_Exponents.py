...
popt_T,pcov_T = curve_fit(TcScaling, invL, TcList, [5,0.89])#, bounds = bounds)
...
if choice == 'Ratio':
    ...
    ax1_f2.text(0.25,19.5,'Expected Bounds $\pm$ 8.5',fontsize = 20)
...
if choice == 'Ratio':
    ...
    ax1_f3.text(0.25,1.33,'Expected Bounds $\pm$ 0.977', fontsize = 20)
...
def FitAlpha(L):
    ...
    fig1.suptitle('Specific Heat: %s x %s Square XY' %(L,L),fontsize = 20)
...
def FitGamma(L):
    ...
    ax1.set_xlabel('Temperature', fontsize = 15)
    ax1.set_ylabel('Specific Heat', fontsize = 15)
    fig1.suptitle('Susceptibility: %s x %s Square XY' %(L,L),fontsize = 20)
    ax1.set_xlim([np.amin(Temp), np.amax(Temp)])
