...
gen = 0
if gen == 1:
    Temp,E,M,Cp,Chi=Heisenberg.T_dep(L)
    if anti == 1:
        #Save Data
...
if ferro == 1:
    ...
    ax4.yaxis.set_minor_locator(ticker.MultipleLocator(0.4))

ax1.set_ylim([np.amin(E) - 0.1, np.amax(E) + 0.1])
ax2.set_ylim([-1,1])
ax3.set_ylim([np.amin(Cp) - 0.1, np.amax(Cp) + 0.1])
ax4.set_ylim([np.amin(Chi) - 0.2, np.amax(Chi) + 0.2])
...
fig1.suptitle('Square Ferromagnetic XY Model',fontsize = 20)
