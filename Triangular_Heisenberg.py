...
def MCstep_jit(N, L, T,h, state, acceptedMoves, energy, magnetization):
    ...
    for k in range(N):
        ...
        energy_i = -np.cos(state[i,j] - state[(i+1)%L,j]) + -np.cos(state[i,j] - state[(i-1)%L,j]) + -np.cos(state[i,j] - state[i,(j-1)%L]) + -np.cos(state[i,j] - state[i,(j+1)%L]) + -np.cos(state[i,j] - state[(i+1)%L,(j+1)%L]) + -np.cos(state[i,j] - state[(i-1)%L,(j-1)%L])

        dtheta = np.random.uniform(-np.pi,np.pi)
        newSpinTemp = state[i,j] + dtheta

        energy_f = -np.cos(newSpinTemp - state[(i+1)%L,j]) + -np.cos(newSpinTemp - state[(i-1)%L,j]) + -np.cos(newSpinTemp - state[i,(j-1)%L]) + -np.cos(newSpinTemp - state[i,(j+1)%L]) + -np.cos(newSpinTemp - state[(i+1)%L,(j+1)%L]) + -np.cos(newSpinTemp - state[(i-1)%L,(j-1)%L])
...
class TriangularHeisenberg2D (object):
    ...
    def ShowState(self,T):
        X, Y = np.meshgrid(np.arange(0,self.L),np.arange(0,self.L))
        U = np.cos(self.state)
        V = np.sin(self.state)
        plt.ion()
        #axis.quiver(X, Y, U, V, units='width')
        #axis.axis('off')
        #figure.canvas.draw_idle()
        #figure.show()
        Q = plt.quiver(X, Y, U, V, units='width')
        plt.title('T=%.2f'%T +', Scale='+str(self.L)+'x'+str(self.L))
        plt.axis('off')
        plt.draw()
        plt.pause(1)
        plt.clf()
...
def T_dep(Length):
    t_incr = -0.1
    T = 2.8
    model = TriangularHeisenberg2D(temperature = T, L = Length)   # Tc = 2.3
    Cp = []
    Chi = []
    M = []
    E = []
    ...
    #    else:
    #        Ygrid.append(i)
    #Xgrid,Ygrid = np.array(Xgrid), np.array(Ygrid)
