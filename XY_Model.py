...
N = L ** 2
#state = ones((L,L),int)
state = np.full((L,L), 90, dtype = float)
...
def Stepping(N,L,T,h,state,acceptedMoves,energy,magnetization,random = False):
    ...
    if random:
        ...
        dtheta = int(np.degrees(2*np.pi * r)) #Sphere point picking
    else:
        dtheta = 1 #degrees
    for s in Cluster:
        (ii,jj) = s
...
def getEnergy(L,state):
    ...
    for ii in range(L):
        for jj in range(L):
            ...
            for nbr in nbrs:
                (k,l) = nbr
                E += -np.cos((np.deg2rad(state[ii,jj]) - np.deg2rad(state[k,l])))
    return E
...
def Quench(Tmax,Tmin,Tstep,nsteps,N,L,h,state,acceptedMoves,energy,magnetization, show = False, critical = False, run = True):
    ...
    if run == True:
        ...
        #Save Data
        np.save(filepath + 'Temp_Square_Wolff_10.npy', TempList)
        np.save(filepath + 'Energy_Square_Wolff_10.npy', E)
        np.save(filepath + 'Magnetization_Square_Wolff_10.npy', M)
        np.save(filepath + 'Specific_Heat_Square_Wolff_10.npy', Cp)
        np.save(filepath + 'Susceptibility_Square_Wolff_10.npy', Chi)
        ...
        print('Loading Data...')
        TempList,E,M,Cp,Chi = np.load(filepath + 'Temp_Square_Wolff_10.npy'), np.load(filepath + 'Energy_Square_Wolff_10.npy'), np.load(filepath + 'Magnetization_Square_Wolff_10.npy'), np.load(filepath + 'Specific_Heat_Square_Wolff_10.npy'), np.load(filepath + 'Susceptibility_Square_Wolff_10.npy')
    ...
    if show:
        ...
        if critical:
            ...
            bounds = (0.35,0.5)
            popt_s,pcov = curve_fit(SusceptibilityExp_Para,TempList[:num-1],Chi[:num-1], [0.35], bounds = bounds)
            ChiCritical_Para = SusceptibilityExp_Para(TempList[:num-1],*popt_s)
            ax4.plot(TempList[:num-1],ChiCritical_Para-0.75,lw = 3, label = 'Paramagnetic Phase Fit', color = 'steelblue')
        ...
        fig1.suptitle('Ferromagnetic Square Heisenberg Model',fontsize = 20)
